...
export const UdpTrafficChart = memo(() => {
  const { t } = useTranslation();
  const { data: inData,  loading: loadingIn  } = usePrometheus({ query: 'rate(node_udp_indatagrams_total[2m])',  fallbackSeriesName: 'in',  skeletonKeys: ['in']  });
  const { data: outData, loading: loadingOut } = usePrometheus({ query: 'rate(node_udp_outdatagrams_total[2m])', fallbackSeriesName: 'out', skeletonKeys: ['out'] });
    ...
    <div className="card col-span-1">
      <div className="inline-flex items-center mb-4">
        <h3 className="text-lg font-semibold text-[#F8FAFC] flex items-center">
          {t('metrics.udpTraffic')}
          <InfoTooltip text={t('descriptions.udpTraffic')} />
        </h3>
      </div>
      <div className="h-72">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={merged} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            ...
            <Line type="linear" dataKey="in"  name="UDP In/s"  stroke="#22C55E" strokeWidth={2} dot={false} connectNulls isAnimationActive={false} />
            <Line type="linear" dataKey="out" name="UDP Out/s" stroke="#3B82F6" strokeWidth={2} dot={false} connectNulls isAnimationActive={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
...
UdpTrafficChart.displayName = 'UdpTrafficChart';
