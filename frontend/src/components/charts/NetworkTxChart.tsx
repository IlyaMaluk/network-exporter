...
export const NetworkTxChart = memo(({ selectedInterfaces = [] }: { selectedInterfaces?: string[] }) => {
  ...
  const { data, loading } = usePrometheus({
    query: `rate(node_network_transmit_bytes_total${interfaceFilter}[2m])`,
          ...
          <AreaChart data={data} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
            <defs>
              {devices.map((device, idx) => (
                <linearGradient key={`grad-tx-${device}`} id={`color-tx-${device}`} x1="0" y1="0" x2="0" y2="1">
            ...
            {devices.map((device, idx) => (
              <Area key={`tx-${device}`} type="linear" dataKey={String(device)} name={String(device)}
                stroke={colors[idx % colors.length]} fillOpacity={1} fill={`url(#color-tx-${device})`}
...
NetworkTxChart.displayName = 'NetworkTxChart';
